...
vec4 trayce7(vec3 ro, vec3 rd, int s, int d, inout vec2 pMinIt, int id, int _s, out vec3 norm) {
	vec3 n;
	vec2 it = box(ro,rd,vec3(s),n);

	if (it.x < -s * 2 || it.x == -1 || pMinIt.x < it.x) return vec4(0);
...
vec4 trayce6(vec3 ro, vec3 rd, int s, int d, inout vec2 pMinIt, int id, int _s, out vec3 norm) {
	vec3 n;
	vec2 it = box(ro,rd,vec3(s),n);
	...
	for (int i = 0; i < 8; i++) {
		...
		vec4 col = trayce7(ro - vec3(x,y,z) * s, rd, s / 2, d - 1, minIt, id + i * ipow(8, _s - 2), _s - 1, norm);
...
vec4 trayce5(vec3 ro, vec3 rd, int s, int d, inout vec2 pMinIt, int id, int _s, out vec3 norm) {
	vec3 n;
	vec2 it = box(ro,rd,vec3(s),n);
	...
	for (int i = 0; i < 8; i++) {
		...
		vec4 col = trayce6(ro - vec3(x,y,z) * s, rd, s / 2, d - 1, minIt, id + i * ipow(8, _s - 2), _s - 1, norm);
...
vec4 trayce4(vec3 ro, vec3 rd, int s, int d, inout vec2 pMinIt, int id, int _s, out vec3 norm) {
	vec3 n;
	vec2 it = box(ro,rd,vec3(s),n);
	...
	for (int i = 0; i < 8; i++) {
		...
		vec4 col = trayce5(ro - vec3(x,y,z) * s, rd, s / 2, d - 1, minIt, id + i * ipow(8, _s - 2), _s - 1, norm);
...
vec4 trayce3(vec3 ro, vec3 rd, int s, int d, inout vec2 pMinIt, int id, int _s, out vec3 norm) {
	vec3 n;
	vec2 it = box(ro,rd,vec3(s),n);
	...
	for (int i = 0; i < 8; i++) {
		...
		vec4 col = trayce4(ro - vec3(x,y,z) * s, rd, s / 2, d - 1, minIt, id + i * ipow(8, _s - 2), _s - 1, norm);
...
vec4 trayce2(vec3 ro, vec3 rd, int s, int d, inout vec2 pMinIt, int id, int _s, out vec3 norm) {
	vec3 n;
	vec2 it = box(ro,rd,vec3(s),n);
	...
	for (int i = 0; i < 8; i++) {
		...
		vec4 col = trayce3(ro - vec3(x,y,z) * s, rd, s / 2, d - 1, minIt, id + i * ipow(8, _s - 2), _s - 1, norm);
...
vec4 trayce(inout vec3 ro, inout vec3 rd, int s, int d, int _s) {
	...
	for (int i = 0; i < 8; i++) {
		...
		vec4 col = trayce2(ro - vec3(x,y,z) * s, rd, s / 2, d - 1, minIt, i * ipow(8, _s - 2), _s - 1, norm);
